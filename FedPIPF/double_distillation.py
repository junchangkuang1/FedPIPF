...
# Helper func
############################
from double_helper import *
#################################
TRAIN_RATIO = 0.8
RS = 30448  # random state
N_CHANNELS, N_CLASSES = 1, 1
bilinear = True
BATCH_SIZE, EPOCHS = 4, 150
...
for client in CLIENTS:
    ...
    if nets[client]['unet'] is None:
        nets[client]['unet'] = UNet(1, 1).to(device)
...
acc_train = {client: {'medsam': [], 'unet': []} for client in CLIENTS}
loss_train = {client: {'medsam': [], 'unet': []} for client in CLIENTS}
iter_num = {client: {'medsam': 0, 'unet': 0} for client in CLIENTS}
    ...
    for client, supervision_t in zip(CLIENTS, CLIENTS_SUPERVISION):
        if supervision_t == 'unlabeled':
            if not USE_UNLABELED_CLIENT:
                acc_train[client]['unet'].append(0)
                loss_train[client]['unet'].append(0)
                continue

        if client != 'Interobs' and client != 'Lung1':
            print(f"Current client: {client}")
            # MedSAM 训练
            acc_medsam,loss_medsam= train_model(epoch, training_clients[client], optimizers[client]['medsam'], device, nets[client]['medsam'],acc=acc_train[client]['medsam'], loss=loss_train[client]['medsam'],
                        supervision_type='labeled', learning_rate=LR,iter_num=iter_num[client]['medsam']
,
                        model_type='medsam')
            # UNet 训练
            acc_unet,loss_unet=train_model(epoch, training_clients[client], optimizers[client]['unet'], device, nets[client]['unet'],
                        acc=acc_train[client]['unet'], loss=loss_train[client]['unet'],
                        supervision_type='labeled', learning_rate=LR, iter_num=iter_num[client]['unet']
,
                        model_type='unet')

            epoch_loss.append(loss_unet)
    ...
    aggr_nets = {client: nets[client]['unet'] for client in CLIENTS}
    aggr_nets['global'] = nets['global']['unet']
    aggr_fed(CLIENTS, WEIGHTS, aggr_nets)
    ################### test ################################
    avg_acc = 0.0
    for order, (client, supervision_t) in enumerate(zip(CLIENTS, CLIENTS_SUPERVISION)):
        ...
        if client == 'GX':
            continue
        acc_test,jc,assd,hd95 = test(testing_clients[client], nets['global']['unet'], device, acc_valid[client], loss_test[client])
